...
const LAWS = [
...

  {
    ...
    tagline: 'Systems that tune themselves to the edge of catastrophe.',
    equation: 'P(s) \\sim s^{-\\tau}',
    deps: ['complexity', 'statmech'], sim: null,
    eli5: `Trickle sand onto a tabletop, one grain at a time. At first it just piles up. Then the sides get steep, and grains start sliding — a few at a time, sometimes a tiny slip, sometimes a whole face of the pile letting go at once. Keep dripping and the pile settles into a special slope where it stays balanced right on the edge of collapsing. From then on, the *same* falling grain might cause nothing, or a tiny slide, or a giant avalanche — and there's no way to tell which in advance. Nobody set up that knife-edge slope on purpose. The pile found it all by itself, just by being fed.`,
    intermediate: `Most things in physics only get dramatic when you tune them carefully — water boils at exactly 100 °C, a magnet loses its magnetism at exactly its Curie temperature. These "critical points," where tiny nudges cause system-wide effects, normally have to be dialled in by hand. In 1987 Per Bak, Chao Tang and Kurt Wiesenfeld asked a startling question: what if some systems *drive themselves* to a critical point and stay there, with no tuning at all? Their toy model was a sandpile. Drop grains randomly; whenever a column gets too tall it topples onto its neighbours, which can topple in turn, making avalanches. Left alone, the pile organises itself into a state where avalanche sizes follow a power law — no typical size, just lots of small slides and rare huge ones, with every scale in between. The same statistics show up in earthquakes, forest fires, mass extinctions, stock-market crashes, and the crackle of crumpling paper. Bak's slogan was "how nature works": complexity and catastrophe as the unforced, automatic destination of slowly-driven systems.`,
    expert: `Self-organised criticality (SOC) describes slowly-driven, dissipative, spatially-extended systems whose dynamics flow without parameter tuning to a stationary state with no characteristic length or time scale. The Bak–Tang–Wiesenfeld (BTW) model on a 2D square lattice assigns each site a height z; add grains randomly, and when z ≥ z_c = 4 the site topples, z → z − 4 with one grain to each of its four neighbours, dissipating only at open boundaries. The separation of timescales — slow drive, fast relaxation — is essential: drive ≪ avalanche dynamics ≪ dissipation. Avalanche observables (size s, area, duration t, linear extent) obey power laws P(s) ∼ s^{−τ}, P(t) ∼ t^{−α}, cut off only by system size (finite-size scaling), with the BTW exponents τ ≈ 1.27 in 2D. Dhar (1990) showed the model is *abelian* — the final stable configuration is independent of toppling order — and exactly solvable via the sandpile group acting on recurrent configurations, whose count equals det(Δ) of the graph Laplacian. SOC links to ordinary criticality through absorbing-state phase transitions (Manna universality, conserved directed percolation): the slow drive plus dissipation self-tunes the system to the critical density of an underlying transition. Real-world signatures include the Gutenberg–Richter earthquake law, 1/f noise, Barkhausen noise in magnets, solar-flare energy distributions, and neuronal avalanches in cortex (Beggs & Plenz, 2003). Whether any given empirical power law is genuine SOC versus mere heavy-tailed statistics remains contested — fitting power laws is notoriously easy to do badly.`,
    surprise: `The very brain reading this sentence may live permanently on the knife-edge. Recordings of cortical tissue show cascades of neural firing — "neuronal avalanches" — whose sizes follow the same power law as Bak's sandpile, with an exponent near −3/2. The leading hypothesis is that a brain poised exactly at criticality maximises its dynamic range, information storage and transmission. Too quiet and signals die; too excitable and they blow up into a seizure. Thought may be the sound of a sandpile that has tuned itself to the brink, forever about to avalanche.`,
    history: `Per Bak, Chao Tang and Kurt Wiesenfeld introduced the idea in a 1987 *Physical Review Letters* paper, "Self-organized criticality: An explanation of 1/f noise," written at Brookhaven National Laboratory. The sandpile was a metaphor that escaped the lab; Bak, a famously combative Dane, spent the next decade arguing that SOC was nothing less than the reason the world is complicated at all, collected in his 1996 popular book *How Nature Works*. Deepak Dhar made the abelian sandpile exactly solvable in 1990, turning a physicist's cartoon into a rich object of combinatorics and probability theory still studied today.`,
  },
...
const IMAGES = {
  ...
  soc:           { image: WM('Sandpile_10000x10000_identity_configuration.png'),
                   caption: 'The abelian sandpile identity — a fractal that emerges, untuned, from the toppling rule' },
