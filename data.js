...
const LAWS = [
...

  {
    ...
    expert: `Modern CODATA value G = 6.67430(15) × 10⁻¹¹ N·m²/kg² — only five significant figures, the worst-known fundamental constant. Independent torsion-pendulum experiments disagree at the ~10⁻⁴ level, well beyond their stated uncertainties; the resolution is unknown and is its own active sub-field. Newton's gravity is the weak-field static limit of Einstein's general relativity: the Newtonian potential Φ satisfies Poisson's equation ∇²Φ = 4πGρ, the c → ∞ limit of the Einstein field equations. The 1/r² force admits two hidden integrals of motion — angular momentum 𝐋 and the Laplace–Runge–Lenz vector 𝐀 — closing each bound orbit into an exact non-precessing ellipse; Bertrand's theorem (1873) proves that *only* inverse-square and Hooke (∝ r) potentials produce closed orbits for every bound trajectory. Tidal acceleration scales as ∂g/∂r ∝ M/r³, which is why the Moon raises tides on Earth roughly 2.2× larger than the Sun does despite the Sun being 27 million times more massive. The equivalence principle (m_inertial = m_gravitational), tested to ~10⁻¹⁵ by the MICROSCOPE satellite (2022), was Einstein's empirical seed for GR. Mercury's anomalous perihelion advance — 43″ per century beyond Newton's prediction — was the first known failure of the law, resolved by GR in 1915. Open problems: dark matter (galaxy rotation curves), dark energy (Λ ~ 10⁻¹²³ in Planck units), and the quantum description of gravity at the Planck scale ℓ_P = √(ℏG/c³) ≈ 1.6 × 10⁻³⁵ m.`,
